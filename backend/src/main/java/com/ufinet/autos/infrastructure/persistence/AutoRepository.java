...
/**
 * Spring Data JPA repository for {@link AutoEntity}.
 * All queries are scoped by the owner's user ID.
 */
@Repository
public interface AutoRepository extends JpaRepository<AutoEntity, Long> {
     ...
     * @return filtered list of car entities owned by that user
     */
    ...
    List<AutoEntity> searchByUserId(
    ...
    /**
     * Returns a single car entity by ID, only if it belongs to the given user.
     *
     * @param id     the car's primary key
     * @param userId the owner's primary key
     * @return the entity if found and owned by that user, or empty
     */
    @Query("SELECT a FROM Auto a WHERE a.id = :id AND a.user.id = :userId")
    Optional<AutoEntity> findByIdAndUserId(@Param("id") Long id, @Param("userId") Long userId);
}
